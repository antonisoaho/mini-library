    ...
    for (const key in book) {
        if (book.hasOwnProperty(key)) {
            ...
            if (property.htmlType) {
                ...
                const detailsSection = details[property.type];
                if (detailsSection)
                    detailsSection.append(propertyElement);
            }
        }
    }
    for (const section in details) {
        if (details.hasOwnProperty(section))
            infoPage.append(details[section]);
    }
    return infoPage;
};
const showModalInfo = (book) => {
